...
class WeekLunchMenuView(APIView):
    ...
    def get(self, request):
        summary_mode = request.GET.get('summary', 'false').lower() == 'true'
        ...
        for menu in daily_menus:
            ...
            if summary_mode:
                ...
                menu_data = self._get_menu_summary(menu, request)
            else:
                # Return full menu data
            ...
            # STRICT: Only include menus that have dishes or complexes
            if summary_mode:
                # For summary, check if we have at least soup or main dish
                if menu_data.get('soup') or menu_data.get('main_dish'):
                    week_menus.append(menu_data)
            else:
                # For full menu, check categories
